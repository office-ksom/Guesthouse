...
// .wrangler/tmp/bundle-ZMwmXl/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-ZMwmXl/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
// .wrangler/tmp/bundle-ZMwmXl/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
async function sendGroupApprovalEmail(recipient, groupRef, bookings2, senderEmail, senderName) {
  ...
  const bookingRows = bookings2.map((b, idx) => {
...
async function triggerBookingEmail(c, bookingId, oldStatus, newStatus, senderEmail, senderName) {
  ...
  try {
    ...
    if (booking.group_ref) {
      ...
      if (newStatus === "confirmed") {
        const guestObj = await c.env.DB.prepare("SELECT name, email, designation FROM guests WHERE id = ?").bind(booking.guest_id).first();
        let rName = "Not Allotted";
        if (booking.room_id) {
          const roomObj = await c.env.DB.prepare("SELECT name FROM rooms WHERE id = ?").bind(booking.room_id).first();
...
__name(triggerBookingEmail, "triggerBookingEmail");
