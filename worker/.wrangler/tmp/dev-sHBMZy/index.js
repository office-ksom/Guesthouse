...
// .wrangler/tmp/bundle-QLWCrU/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-QLWCrU/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
// .wrangler/tmp/bundle-QLWCrU/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
rooms.get("/:id", async (c) => {
  const room = await c.env.DB.prepare("SELECT * FROM rooms WHERE id = ? AND is_active = 1").bind(parseInt(c.req.param("id") || "0")).first();
  if (!room)
    ...
    try {
      ...
      if (existing.group_ref) {
        ...
        if (otherPending.length === 0) {
          ...
          for (const b of allGroupBookings) {
            if (b.guest_email && b.guest_email !== sponsorMail) {
              await sendGroupApprovalEmail(b.guest_email, null, [b]);
            }
          }
        }
      } else {
        const guestObj = await c.env.DB.prepare("SELECT name, email, designation FROM guests WHERE id = ?").bind(existing.guest_id).first();
        ...
        if (guestObj?.email && guestObj.email !== sponsorMail) {
          await sendGroupApprovalEmail(guestObj.email, null, [bInfo]);
        }
      }
    } catch (err) {
