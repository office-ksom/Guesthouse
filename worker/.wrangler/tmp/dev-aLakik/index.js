...
// .wrangler/tmp/bundle-Vh1M3i/strip-cf-connecting-ip-header.js
...
function decodeBase64url(str) {
  ...
  const raw2 = atob(base64);
  return Uint8Array.from([...raw2].map((c) => c.charCodeAt(0)));
}
...
async function signJWT(payload, secret) {
  ...
  const fullPayload = { ...payload, iat: now, exp: now + 8 * 60 * 60 };
  const enc = new TextEncoder();
...
// .wrangler/tmp/bundle-Vh1M3i/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
// .wrangler/tmp/bundle-Vh1M3i/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
auth.post("/login", async (c) => {
  ...
  const user = await c.env.DB.prepare("SELECT * FROM staff_users WHERE email = ? AND is_active = 1").bind(body.email.toLowerCase().trim()).first();
  if (!user) {
  ...
  await c.env.DB.prepare("UPDATE staff_users SET last_login = datetime('now') WHERE id = ?").bind(user.id).run();
...
auth.post("/google-login", async (c) => {
  ...
  const staff = await c.env.DB.prepare("SELECT id, name, role FROM staff_users WHERE email = ? AND is_active = 1").bind(email).first();
  let role = "sponsor";
  let userId = email;
  if (staff) {
    role = staff.role;
...
rooms.get("/", async (c) => {
  ...
  const activeBookings = await c.env.DB.prepare("SELECT room_id, status FROM bookings WHERE ? >= check_in_date AND ? < check_out_date AND status IN ('confirmed', 'checked_in')").bind(today, today).all();
  ...
  const rooms2 = results.map((r) => ({
    ...r,
    amenities: JSON.parse(r.amenities || "[]"),
    photos: JSON.parse(r.photos || "[]"),
    status: statusMap.get(r.id) || "Vacant"
  }));
  return c.json({ rooms: rooms2 });
});
rooms.get("/:id", async (c) => {
  const room = await c.env.DB.prepare("SELECT * FROM rooms WHERE id = ? AND is_active = 1").bind(parseInt(c.req.param("id"))).first();
  if (!room)
    return c.json({ error: "Room not found" }, 404);
  return c.json({ room: { ...room, amenities: JSON.parse(room.amenities || "[]"), photos: JSON.parse(room.photos || "[]") } });
});
rooms.post("/", authMiddleware("admin"), async (c) => {
  const body = await c.req.json();
  if (!body.name || !body.type || !body.capacity || body.price_per_night === void 0) {
...
rooms.put("/:id", authMiddleware("admin"), async (c) => {
  ...
  const body = await c.req.json();
  const existing = await c.env.DB.prepare("SELECT * FROM rooms WHERE id = ?").bind(id).first();
  if (!existing)
    return c.json({ error: "Room not found" }, 404);
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
function generateBookingRef(id, year) {
...
function generateInvoiceNumber(id, year) {
  const y = year ?? (/* @__PURE__ */ new Date()).getFullYear();
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
async function sendGroupApprovalEmail(recipient, groupRef, bookings2) {
  if (!recipient) {
    console.warn("[Email System] No recipient email specified. Skipping email.");
    return;
  }
  const subject = groupRef ? `KSoM Guesthouse Booking Approval - Group Request ${groupRef}` : `KSoM Guesthouse Booking Approval - ${bookings2[0]?.booking_ref || "Confirmation"}`;
  let emailContent = `
========================================================================
\u2709\uFE0F  EMAIL SENT TO: ${recipient}
\u{1F4C2}  SUBJECT: ${subject}
========================================================================
...
------------------------------------------
- Name: ${b.guest_name}
${b.designation ? `- Designation: ${b.designation}
` : ""}- Reference: ${b.booking_ref}
- Stay Dates: ${b.check_in_date} to ${b.check_out_date}
- Room Allotted: ${b.room_name || "Not Allotted"}
- Allotment Status: ${statusText}
`;
  });
  emailContent += `
------------------------------------------
...
Thank you,
KSoM Guesthouse Administration
...
bookings.post("/", authMiddleware("admin", "receptionist", "sponsor"), async (c) => {
  ...
  const body = await c.req.json();
  const guestsList = body.guests && Array.isArray(body.guests) ? body.guests : body.guest ? [body.guest] : [];
  ...
  const nights = Math.ceil((checkOut.getTime() - checkIn.getTime()) / 864e5);
  let roomId = body.room_id ? parseInt(body.room_id) : null;
  let total = 0;
  const isRentWaived = body.is_rent_waived !== void 0 ? body.is_rent_waived : 1;
  if (roomId) {
    const room = await c.env.DB.prepare("SELECT * FROM rooms WHERE id = ? AND is_active = 1").bind(roomId).first();
    if (!room)
      return c.json({ error: "Room not found or unavailable" }, 404);
    ...
    if (conflict)
      return c.json({ error: "Room is not available for selected dates" }, 409);
    if (isRentWaived === 0) {
      total = nights * room.price_per_night;
    }
  }
  const sponsorEmail = user.role === "sponsor" ? user.email : body.sponsor_email || null;
  ...
  for (const g of guestsList) {
    ...
    `).bind(
      ...
      body.check_out_date,
      body.num_guests ?? 1,
      body.purpose_of_visit ?? null,
      body.special_requests ?? null,
      total,
      body.requested_type ?? null,
      body.requested_ac ?? null,
      sponsorEmail,
      isRentWaived
    ).run();
  ...
  return c.json({
    ...
    booking_ref: createdBookings[0]?.booking_ref,
...
bookings.get("/", authMiddleware("admin", "receptionist", "viewer", "sponsor"), async (c) => {
  ...
  if (date) {
    query += " AND (b.check_in_date = ? OR b.check_out_date = ?)";
...
bookings.put("/:id", authMiddleware("admin", "receptionist"), async (c) => {
  ...
  if (!existing)
    return c.json({ error: "Booking not found" }, 404);
  let roomId = body.room_id !== void 0 ? body.room_id : existing.room_id;
  let totalAmount = body.total_amount !== void 0 ? body.total_amount : existing.total_amount;
  let isRentWaived = body.is_rent_waived !== void 0 ? body.is_rent_waived : existing.is_rent_waived;
  if (roomId && roomId !== existing.room_id) {
    ...
    if (conflict)
      return c.json({ error: "Room is not available for selected dates" }, 409);
    if (isRentWaived === 0 && body.total_amount === void 0) {
  ...
  if (body.status === "confirmed" && existing.status !== "confirmed") {
    try {
      const sponsorMail = existing.sponsor_email;
      if (sponsorMail) {
        if (existing.group_ref) {
          const { results: groupBookings } = await c.env.DB.prepare(`
              SELECT b.id, b.status, b.booking_ref, b.check_in_date, b.check_out_date,
                     g.name as guest_name, g.designation, r.name as room_name
              FROM bookings b
              JOIN guests g ON b.guest_id = g.id
              LEFT JOIN rooms r ON b.room_id = r.id
              WHERE b.group_ref = ?
            `).bind(existing.group_ref).all();
          const otherPending = groupBookings.filter((b) => b.id !== id && b.status === "pending");
          if (otherPending.length === 0) {
            const allGroupBookings = [...groupBookings];
            const currentIdx = allGroupBookings.findIndex((b) => b.id === id);
            if (currentIdx !== -1) {
              allGroupBookings[currentIdx].status = "confirmed";
              if (roomId) {
                const roomObj = await c.env.DB.prepare("SELECT name FROM rooms WHERE id = ?").bind(roomId).first();
                allGroupBookings[currentIdx].room_name = roomObj?.name || "Allotted";
              }
            }
            await sendGroupApprovalEmail(sponsorMail, existing.group_ref, allGroupBookings);
          }
        } else {
          const guestObj = await c.env.DB.prepare("SELECT name, designation FROM guests WHERE id = ?").bind(existing.guest_id).first();
          ...
          await sendGroupApprovalEmail(sponsorMail, null, [bInfo]);
        }
      }
    } catch (err) {
...
guests.get("/:id", authMiddleware("admin", "receptionist", "viewer"), async (c) => {
  const guest = await c.env.DB.prepare("SELECT * FROM guests WHERE id = ?").bind(parseInt(c.req.param("id"))).first();
  if (!guest)
    return c.json({ error: "Guest not found" }, 404);
  const { results: bookings2 } = await c.env.DB.prepare(`
  ...
  return c.json({ guest, bookings: bookings2 });
});
guests.put("/:id", authMiddleware("admin", "receptionist"), async (c) => {
  const id = parseInt(c.req.param("id"));
  const body = await c.req.json();
  const existing = await c.env.DB.prepare("SELECT * FROM guests WHERE id = ?").bind(id).first();
  if (!existing)
    return c.json({ error: "Guest not found" }, 404);
  await c.env.DB.prepare(
...
availability.get("/", async (c) => {
  ...
  if (roomId) {
    roomQuery += " AND id = ?";
    roomParams.push(parseInt(roomId));
  }
  const { results: rooms2 } = await c.env.DB.prepare(roomQuery).bind(...roomParams).all();
  let bookingQuery = `
    ...
    AND check_in_date < ? AND check_out_date > ?
  `;
  const bookingParams = [end, start];
  if (roomId) {
    bookingQuery += " AND room_id = ?";
    bookingParams.push(roomId);
  }
  const { results: bookings2 } = await c.env.DB.prepare(bookingQuery).bind(...bookingParams).all();
  const result = rooms2.map((room) => {
    const roomBookings = bookings2.filter((b) => b.room_id === room.id);
...
invoices.post("/", authMiddleware("admin", "receptionist"), async (c) => {
  ...
  const booking = await c.env.DB.prepare("SELECT * FROM bookings WHERE id = ?").bind(body.booking_id).first();
  if (!booking)
    return c.json({ error: "Booking not found" }, 404);
  ...
  sections_index: JSON.stringify([
    {
      ...
      content: {
        ...
        btn1_label: "\u{1F511} Login to Book Rooms",
    ...
    },
    {
      ...
      content: {
        ...
        cards: [
          { icon: "\u{1F3DB}\uFE0F", title: "On-Campus Location", desc: "Walk to lecture halls, libraries, and seminar rooms. No commute, just scholarship." },
          { icon: "\u{1F4F6}", title: "High-Speed Internet", desc: "Access the KSoM campus network for uninterrupted research and remote collaboration." },
          { icon: "\u{1F33F}", title: "Serene Campus", desc: "Nestled in Kerala's lush greenery \u2014 a peaceful environment perfect for deep focus." },
          { icon: "\u{1F4B0}", title: "Affordable Rates", desc: "Government-institute pricing \u2014 excellent value without compromising on comfort." },
          { icon: "\u{1F510}", title: "Safe & Secure", desc: "24/7 campus security, secure access, and dedicated reception support for all guests." },
          { icon: "\u{1F4CB}", title: "Easy Booking", desc: "Book online in minutes and receive your confirmation instantly \u2014 no paperwork." }
        ]
      }
    },
    {
    ...
    },
    {
      ...
      content: {
        ...
        cards: [
          ...
          { title: "\u{1F4B3} Payment", points: ["Payment at check-out", "Cash, UPI, bank transfer accepted", "Invoice provided on request"] },
          { title: "\u{1F4CB} Guest Eligibility", points: ["KSoM programme participants", "Invited faculty & researchers", "External guests: prior approval required"] }
        ]
      }
    }
  ]),
...
settings.get("/", async (c) => {
  try {
    ...
    const settingsObj = { ...DEFAULT_SECTIONS };
    results.forEach((row) => {
...
settings.put("/", authMiddleware("admin"), async (c) => {
  try {
    ...
    const statements = Object.entries(body).map(([key, val]) => {
      return c.env.DB.prepare("INSERT OR REPLACE INTO site_settings (key, value) VALUES (?, ?)").bind(key, typeof val === "string" ? val : JSON.stringify(val));
    });
...
app.route("/api/rooms", rooms_default);
app.route("/api/bookings", bookings_default);
app.route("/api/guests", guests_default);
app.route("/api/availability", availability_default);
app.route("/api/invoices", invoices_default);
...
app.get("/api/stats", authMiddleware("admin", "receptionist", "viewer"), async (c) => {
  ...
  const [arrivals, departures, occupied, totalRevenue, pending] = await Promise.all([
    c.env.DB.prepare("SELECT COUNT(*) as count FROM bookings WHERE check_in_date = ? AND status = 'confirmed'").bind(today).first(),
    c.env.DB.prepare("SELECT COUNT(*) as count FROM bookings WHERE check_out_date = ? AND status = 'checked_in'").bind(today).first(),
    c.env.DB.prepare("SELECT COUNT(*) as count FROM bookings WHERE status = 'checked_in'").first(),
    c.env.DB.prepare("SELECT COALESCE(SUM(amount),0) as total FROM invoices WHERE payment_status = 'paid'").first(),
    c.env.DB.prepare("SELECT COUNT(*) as count FROM bookings WHERE status = 'pending'").first()
  ]);
  const totalRooms = await c.env.DB.prepare("SELECT COUNT(*) as count FROM rooms WHERE is_active = 1").first();
  return c.json({
